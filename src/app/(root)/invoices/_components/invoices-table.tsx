"use client";

import * as React from "react";
import { getInvoicesTableColumns } from "./invoices-table-columns";
import { DataTable } from "@/components/data-table/data-table";
import { useDataTable } from "@/hooks/use-data-table";
import { InvoiceListItem } from "@/types";
import { DataTableToolbar } from "@/components/data-table/data-table-toolbar";
import { Badge } from "@/components/ui/badge";
import { CreateInvoiceRedirectButton } from "./create-invoice-redirect-button";
import { DataTableRowAction } from "@/types/data-table";
import { DeleteInvoiceDialog } from "./delete-invoice-dialog";

export function InvoicesTable({ data }: { data: InvoiceListItem[] }) {
  const [rowAction, setRowAction] = React.useState<DataTableRowAction<InvoiceListItem> | null>(
    null
  );

  const columns = React.useMemo(() => getInvoicesTableColumns(setRowAction), []);

  const { table } = useDataTable({
    data,
    columns,
  });

  return (
    <>
      <DataTable table={table}>
        <DataTableToolbar table={table}>
          <Badge className="bg-emerald-600/10 dark:bg-emerald-600/20 hover:bg-emerald-600/10 text-emerald-500 border-emerald-600/60 border-dashed shadow-none rounded-md h-8">
            <div className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
            {data.length} kayıt bulundu.
          </Badge>
          <CreateInvoiceRedirectButton />
        </DataTableToolbar>
      </DataTable>
      {rowAction?.variant === "delete" && (
        <DeleteInvoiceDialog
          open
          onOpenChange={() => setRowAction(null)}
          invoice={rowAction?.row.original ?? null}
          showTrigger={false}
          onSuccess={() => rowAction?.row.toggleSelected(false)}
        />
      )}
    </>
  );
}
